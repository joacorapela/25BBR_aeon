...
Long-duration, continual data series pose particular analytic challenges,
especially in settings where the statistical properties of the data vary with
time.  We will address these challenges by developing a new NaLoDuCo-focused
environment of machine-learning analysis methods specifically tailored to: (1)
operate effectively under non-stationary conditions, and (2) scale to very long
time series.

\subsubsubsection*{Base analysis methods}

Our first goal will be to provide robust and scalable implementations of
methods that are already used at the GCNU, SWC, and AIND to analyse neural and
behavioural time series.  These methods span multiple domains—kinematics,
neural dynamics, behavioural state segmentation, forecasting, and joint
modelling—and are designed to work together within an integrated analysis
pipeline.
